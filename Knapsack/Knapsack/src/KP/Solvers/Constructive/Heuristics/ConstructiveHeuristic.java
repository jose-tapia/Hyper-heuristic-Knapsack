...
/**
 * Provides the methods to create and use constructive heuristics to solve
 * knapsack problems.
 * <p>
...
public class ConstructiveHeuristic {
    ...
    public enum Heuristic {
        ...
        MAX_PROFIT,
        /**
         * Packs the item closest the average profit.
         */
        AVG_PROFIT,
        /**
        ...
        /**
         * Packs the item with the highest rate profit^2/weight.
         */
        MAX_PROFIT2_PER_WEIGHT_UNIT,
        /**
         * Packs the item with the highest rate profit/weight^2.
         */
     ...
     * <p>
     * @param heuristic The selection heuristic to be used by this constructive
     * heuristic.
     */
    ...
    public Item nextItem(Knapsack knapsack, List<Item> items) {
        ...
        switch (heuristic) {
            ...
            case MAX_PROFIT_PER_WEIGHT_UNIT:
                ...
                return selected;
            case MAX_PROFIT2_PER_WEIGHT_UNIT:
                value = -Double.MAX_VALUE;
                for (Item item : items) {
                    if (knapsack.canPack(item) && item.getProfit() * item.getProfitPerWeightUnit() > value) {
                        selected = item;
                        value = item.getProfit() * selected.getProfitPerWeightUnit();
                    }
                }
                return selected;
            case MAX_PROFIT_PER_WEIGHT_UNIT2:
                value = -Double.MAX_VALUE;
                for (Item item : items) {
                    if (knapsack.canPack(item) && item.getProfitPerWeightUnit() / item.getWeight() > value) {
                        selected = item;
                        value = selected.getProfitPerWeightUnit() / item.getWeight();
                    }
                }
                return selected;
            case MIN_WEIGHT:
            ...
            case AVG_PROFIT:
                ...
                avg = avg / items.size();
                for (Item item : items) {
                    if (knapsack.canPack(item) && Math.abs(item.getProfit() - avg) < value) {
                        selected = item;
                        value = Math.abs(item.getProfit() - avg);
                    }
                }
                return selected;
        }
